...
int main(int argc, char **argv) {
    ...
    int cur_section = 0;
    int cur_item = 0;
    float scroll_y = 0.0f; // smoothed scroll offset (in pixels) for the item list
    bool running = true;
    char footer[256] = "";
    ...
    while (running && appletMainLoop()) {
        ...
        if (!loaded) {
            ...
            // ----- item list for current section, with smooth scrolling -----
            Section *sec = &sections[cur_section];
            int listTop = 196;
            int listBottom = SCREEN_H - 150;
            int rowH = 56;
            int visibleH = listBottom - listTop;

            // figure out where the selected item should be, and smoothly ease scroll_y towards it
            float targetScroll = (float)(cur_item * rowH) - (float)(visibleH - rowH) / 2.0f;
            float maxScroll = (float)(sec->item_count * rowH - visibleH);
            if (maxScroll < 0) maxScroll = 0;
            if (targetScroll < 0) targetScroll = 0;
            if (targetScroll > maxScroll) targetScroll = maxScroll;
            scroll_y += (targetScroll - scroll_y) * 0.25f;
            if (fabsf(targetScroll - scroll_y) < 0.5f) scroll_y = targetScroll;

            // clip drawing to the list area so rows don't bleed into the header/footer while scrolling
            SDL_Rect clipRect = { 0, listTop, SCREEN_W, visibleH };
            SDL_RenderSetClipRect(renderer, &clipRect);

            for (int i = 0; i < sec->item_count; i++) {
                int y = listTop + (int)(i * rowH - scroll_y);
                if (y + rowH < listTop || y > listBottom) continue; // outside visible area, skip drawing
            ...
            // thin scrollbar on the right edge of the list, only shown when scrolling is actually possible
            if (maxScroll > 0) {
                int barX = SCREEN_W - 30;
                SDL_SetRenderDrawColor(renderer, tabBg.r, tabBg.g, tabBg.b, 255);
                SDL_Rect barTrack = { barX, listTop, 6, visibleH };
                SDL_RenderFillRect(renderer, &barTrack);

                float thumbH = ((float)visibleH / (float)(sec->item_count * rowH)) * (float)visibleH;
                if (thumbH < 30) thumbH = 30;
                float thumbY = listTop + (scroll_y / maxScroll) * (visibleH - thumbH);

                SDL_SetRenderDrawColor(renderer, accent.r, accent.g, accent.b, 255);
                SDL_Rect barThumb = { barX, (int)thumbY, 6, (int)thumbH };
                SDL_RenderFillRect(renderer, &barThumb);
            }

            if (sec->item_count == 0) {
                draw_text(renderer, font, "(seccion vacia)", 80, listTop, gray);
            }
        }
